...
function buildCSP(cspSource: string) {
    const CSPSource = [
        `default-src ${cspSource} file: https:`,
        `img-src ${cspSource} data: https:`,
        `script-src ${cspSource} 'unsafe-eval' 'self' 'unsafe-inline'`,
...
export class PanelBuilder {
    ...
    private addDoomURI() {
        // const doomUri = vscode.Uri.joinPath(this.context.extensionUri, "webview/doom.wasm");

        const doomUri = this.panel.webview.asWebviewUri(vscode.Uri.file(path.resolve(this.WEBVIEW_PATH, "..", "doom.wasm")));

        this.html = this.html.replace(/%DOOMURI%/, doomUri.toString());
    };
}